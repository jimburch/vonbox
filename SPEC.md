...
**▶️ Phase 3 — MQTT to Home Assistant (core working).** A tap plays the mapped movie from cold sleep; switching movies and re-tap-no-op work. The play path was reworked to a Plex deep link after tvOS 26.5 broke `select_source` (sub-milestone 7 / [ADR 0004](./docs/adr/0004-play-plex-via-deep-link-instead-of-select-source.md)). Remaining: the paused/standby state-watcher automation.
...
- ✅ Sub-milestone 5: tap-to-play working. HA automation `test/home-assistant/vonbox_play_movie.yaml` (renamed from `play_from_tap.yaml`) triggers on `vonbox/nfc/tapped`, looks up the Plex rating key from an inline UID→rating-key `variables:` map, and drives the Apple TV. Guards: unknown UID → `error`/`unknown_tag`; already-playing this movie → `already_playing` no-op (never restarts). Verified end-to-end from cold sleep: NTAG215 `045AED5ECD2A81` → The Super Mario Bros. Movie (rating key `190`). **The play mechanism was reworked 2026-06-15 — see sub-milestone 7.**
- ▶️ Sub-milestone 6 (partial): Bidirectional state sync. The play automation publishes `loading`/`playing`/`already_playing`/`error` to `vonbox/state`; the Pico (`test/offline-harness/full_loop_test.py` + `lib/feedback.py`) renders them — verified live. Still to build: the state-watcher automation that watches `media_player.living_room` and republishes `paused`/`standby` on external changes (Siri-Remote pause, Apple TV sleep).
- ✅ Sub-milestone 7 (2026-06-15): cold-start regression diagnosed + fixed. The Apple TV updated to **tvOS 26.5**, which broke pyatv's Companion app-list (`FetchLaunchableApplicationsEvent` times out — [home-assistant/core#171666](https://github.com/home-assistant/core/issues/171666)); `select_source: Plex` silently no-ops, so the old Phase 1 path can't foreground Plex from cold. Reworked to launch+play in one shot via a **Plex deep link** (`media_player.play_media` url on `media_player.living_room`, routed through `launch_app`, which still works). Start-from-the-beginning via a `rest_command` calling Plex `/:/unscrobble`. Full decision + ruled-out alternatives in [ADR 0004](./docs/adr/0004-play-plex-via-deep-link-instead-of-select-source.md); specifics in lessons #13–#17. **One open item:** the `rest_command` re-wire awaits a single live confirmation tap.
...
12. **Retry `play_media` only on genuine timeout, and confirm by rating key.** A fixed delay after `play_media` re-sent the command while the player state simply hadn't flipped yet — and Plex interprets a duplicate `play_media` for the same item as "restart the movie." The working pattern: `wait_template` until the Plex entity reports `playing` *and* its `media_content_id` matches the rating key (15 s timeout), re-send only if that wait times out, max 3 attempts. Was in `play_from_tap.yaml`.

> **⚠️ Lessons #1, #3, #6, #11, #12 describe the original `select_source` + Plex-client
> `play_media` path, which tvOS 26.5 broke (lesson #13).** The current automation no
> longer uses `select_source`, the Companion warm-up poke, or the rating-key retry
> loop — it plays via a Plex deep link. See [ADR 0004](./docs/adr/0004-play-plex-via-deep-link-instead-of-select-source.md)
> and lessons #13–#17. Rating keys (lesson #1) are still correct and central.

13. **tvOS 26.5 silently broke the Apple TV Companion app-list — and with it `select_source`.** After the Apple TV updated to tvOS 26.5, HA's Apple TV integration began timing out on `FetchLaunchableApplicationsEvent` (app list) and `FetchAttentionState` (power state) — a known, **open, unfixed** upstream bug ([home-assistant/core#171666](https://github.com/home-assistant/core/issues/171666), [#168210](https://github.com/home-assistant/core/issues/168210)), affecting pyatv 0.17.0 / HA 2026.4.4 *and newer*. Symptoms: `media_player.living_room` has **no `source_list`**; `select_source: Plex` silently no-ops; the log shows `Failed to update app list … ProtocolError: Command FetchLaunchableApplicationsEvent failed`. **A clean Apple-TV integration re-pair does NOT fix it** (rules out stale credentials) — but `media_player.turn_on` and `remote.send_command` still work (only app-list + attention broke). tvOS can't be downgraded. This is what silently killed the previously-working Phase 1 cold start; everything below is the workaround.
14. **The Plex-for-Apple-TV client only registers with the server while a movie is actively PLAYING.** With `select_source` dead, nothing foregrounds Plex on a cold tap, so the Apple TV wakes to the home screen and `media_player.plex_plex_for_apple_tv_apple_tv` never leaves `unavailable`. Confirmed by hand: merely *opening* Plex (idle/browsing) does **not** register the client; only starting playback does. So you can't initiate playback through the Plex client entity from cold — there's no entity to target. (Corollary: a "patient retry" of `play_media` can't work cold — it would retry into `unavailable` forever.)
15. **Play Plex from cold via a deep link, not the Plex client.** `media_player.play_media` on the **Apple TV** entity (`media_player.living_room`) with `media_content_type: url` and `media_content_id: plex://play/?metadataKey=/library/metadata/<key>&metadataType=1&server=<machineId>` launches Plex AND starts the movie in one shot. It routes through pyatv `launch_app` — a *different* Companion command tvOS 26.5 did **not** break — and needs **no** Plex client registration. The `server=` machine identifier is constant: `curl http://<plex>:32400/identity`. The deep link only fires from an **awake, foregrounded** Apple TV (it can't wake a sleeping box and is a no-op from idle/screensaver), so the automation runs `turn_on` (wakes TV+soundbar via CEC) → `remote.send_command: home` → the deep link. First use shows a **one-time** tvOS "Open Plex?" prompt; granting it once with the Siri Remote is permanent (until an Apple TV reset/re-pair). See [ADR 0004](./docs/adr/0004-play-plex-via-deep-link-instead-of-select-source.md).
16. **Start-from-the-beginning is a server-side reset, not a deep-link param.** tvOS Plex **ignores `viewOffset`** in the deep link (it resumes from the server's stored progress); Plex `/:/progress` enforces a **60-second minimum**, so `time=0` is silently dropped; and this Plex client **doesn't accept `media_player.media_seek`** (its `supported_features` is browse + play_media only — the seek is a no-op). The one lever that works: clear the server resume point with Plex **`/:/unscrobble`** (mark unwatched) *before* the deep link, via an HA `rest_command`. Side effect: the movie shows **unwatched** after each fresh play — fine, even desirable, for a repeat-watch box.
17. **A `rest_command` must be DEFINED or the automation aborts — `continue_on_error` won't save it.** Calling an undefined `rest_command` raises an unknown-action error HA treats as *config*, not *runtime*, so `continue_on_error: true` does **not** swallow it — the automation stops dead at that step (ours aborted before `turn_on`, so nothing even powered on). Add the `rest_command` to `configuration.yaml`, then **Check Configuration → Restart → verify it appears in Developer Tools → Actions**, and test-call it standalone *before* the automation depends on it. Two paste gotchas that bit us: (a) nano's auto-indent put the whole block at column 2 instead of 0 (`expected <block end>` error) — deploy via `cat > configuration.yaml` to paste literally; (b) the long `url:` must be **one line** — a wrapped scalar injects a space and corrupts the query string. The X-Plex-Token is a full-access Plex credential — keep it in `configuration.yaml` on the Pi, never in the repo.
...
- Move from breadboard to the **Adafruit Perma-Proto Half-sized PCB (#571)** — a bussed *clone of a half-size breadboard* (30 rows × A–J, same power rails), **not** a Pico-specific board. Chosen because the `CLAUDE.md` pin map is written in breadboard rows, so the layout transfers hole-for-hole. (A Pico-specific board with the pinout on the silkscreen, e.g. the Pimoroni Pico Proto, was considered and rejected — it's unbussed, has less room for the JST leads, and the row map doesn't transfer; see `docs/production-hardware.md`.) The hole-by-hole build is in `docs/assembly-guide.md`.
